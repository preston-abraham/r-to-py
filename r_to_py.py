...
mode = st.selectbox('Mode',['R to Python','Python to R','Natural language to Python'])

if not mode == 'Natural language to Python':
    ...
    if st.button('Write'):
        response = openai.Completion.create(
                              model="text-davinci-003",
                              prompt = 'Write Python code to do the following: '+code+'\n',
                              temperature = 0.15,
                              top_p = 1,
                              max_tokens = 500
                            )["choices"][0]["text"]
